...
@dataclass
class Image:
    # Directory of .gv file injected as context during parsing
    gv_dir: InitVar[Path]
    ...
    def __post_init__(self, gv_dir):
        ...
        if self.fixedsize is None:
            # Default True if any dimension specified
            # unless self.scale also is specified.
        ...
        if self.fixedsize:
            ...
            if self.height:
                if not self.width:
                    ...
                    self.width *= aspect_ratio(gv_dir.joinpath(self.src))
            else:
                if self.width:
                    self.height = self.width
                    self.height /= aspect_ratio(gv_dir.joinpath(self.src))
    ...
    def __post_init__(self):
        ...
        if self.style == 'simple':
            if self.pincount and self.pincount > 1:
                raise Exception('Connectors with style set to simple may only '
                                'have one pin')
            self.pincount = 1
        ...
        if self.pincount is None:
            if self.pinlabels:
                ...
                raise Exception('You need to specify at least one, pincount, '
                                'pins or pinlabels')
        ...
        # create default lists for pins (sequential) and pinlabels (blank)
        # if not specified
        if not self.pins:
        ...
        for loop in self.loops:
            ...
            # TODO: allow using pin labels in addition to pin numbers, just
            #       like when defining regular connections
...
class Cable:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.gauge, str):  # gauge and unit specified
            try:
                ...
                raise Exception('Gauge must be a number, or number and unit '
        ...
        # number of wires explicitly defined
        if self.wirecount:
            # use custom color palette (partly or looped if needed)
            if self.colors:
                pass
            ...
            # no colors defined, add dummy colors
            else:
            ...
            if not self.colors:
                raise Exception('Unknown number of wires. Must specify '
                                'wirecount or colors (implicit length)')
            self.wirecount = len(self.colors)

        # if lists of part numbers are provided check this is a bundle and that
        # it matches the wirecount.
        for idfield in [self.manufacturer, self.mpn, self.pn]:
            if isinstance(idfield, list):
                if self.category == "bundle":
                    # check the length
                    if len(idfield) != self.wirecount:
                        raise Exception('lists of part data must match '
                                        'wirecount')
                else:
                    raise Exception('lists of part data are only supported for '
                                    'bundles')
    ...
    def connect(self, from_name, from_pin, via_pin, to_name, to_pin):
        ...
        if len(from_pin) != len(to_pin):
            raise Exception('from_pin must have the same number of elements as '
                            'to_pin')
        for i, _ in enumerate(from_pin):
            # self.connections.append((from_name, from_pin[i], via_pin[i],
            # to_name, to_pin[i]))
            self.connections.append(Connection(from_name, from_pin[i],
                                               via_pin[i], to_name, to_pin[i]))
