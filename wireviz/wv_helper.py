...
def nested_html_table(rows):
    ...
    # output: a parent table with one child table per parent item that is list,
    #   and one cell per parent item that is scalar
    # purpose: create the appearance of one table, where cell widths are
    #   independent between rows
    # attributes in any leading <tdX> inside a list are injected into to the
    #   preceeding <td> tag
    html = []
        ...
        if isinstance(row, list):
            if len(row) > 0 and any(row):
                ...
                html.append('  <table border="0" cellspacing="0" '
                            'cellpadding="3" cellborder="1"><tr>')
                for cell in row:
                    if cell is not None:
                        # Inject attributes to the preceeding <td> tag
                        # where needed
                        html.append('   <td balign="left"'
                                    f'>{cell}</td>'.replace('><tdX', ''))
...
def html_colorbar(color):
    if color:
        trans = wv_colors.translate_color(color, "HEX")
        return f'<tdX bgcolor="{trans}" width="4">'
    return None
...
def html_image(image):
    if not image:
        return None
    # The leading attributes belong to the preceding tag. See where used below.
    html = (f'{html_size_attr(image)}>'
            f'<img scale="{image.scale}" '
            f'src="{image.src}"/>')
    if image.fixedsize:
...
def html_caption(image):
    if image and image.caption:
        return f'<tdX sides="BLR">{html_line_breaks(image.caption)}'
    return None


def html_size_attr(image):
    # Return Graphviz HTML attributes to specify minimum
    # or fixed size of a TABLE or TD object
    ret = ''
    if image:
        if image.width:
            ret += f' width="{image.width}"'
        if image.height:
            ret += f' height="{image.height}"'
        if image.fixedsize:
            ret += ' fixedsize="true"'
    return ret
...
def expand(yaml_data):
    ...
    # if str is of the format '#-#', it is treated
    # as a range (inclusive) and expanded
    output = []
...
def flatten2d(inp):
    return [[str(item) if not isinstance(item, list) else ', '.join(item)
             for item in row]
            for row in inp]
...
def aspect_ratio(image_src):
    try:
        ...
        print('aspect_ratio(): Invalid image size '
              f'{image.width} x {image.height}')
    # ModuleNotFoundError and FileNotFoundError are the most expected,
    # but all are handled equally.
